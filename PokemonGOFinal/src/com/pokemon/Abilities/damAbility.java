	...
	public void turn(String enemy) {
		if (count != null) {
			...
			if (count.contains("your-active:damage")) {
				if (enemy.equals("enemy"))
					this.amount = this.amount * ((ObjectHandler.getPlayer().getPoke().getHP()
							- ObjectHandler.getPlayer().getPoke().getCurrentHP()) / 10);
				if (enemy.equals("player"))
					this.amount = this.amount * ((ObjectHandler.getEnemy().getPoke().getHP()
							- ObjectHandler.getEnemy().getPoke().getCurrentHP()) / 10);
			}
		...
		case "choice:opponent-bench":
			if (enemy.equals("enemy") && ObjectHandler.getEnemy().getBench() != null) {
				Game.gameInterface.setSelected(-1);
				Game.gameInterface.setChoiceAbilities(this);
				Game.gameInterface.setChoiceRange(target);
			}
			if (enemy.equals("player")) {
